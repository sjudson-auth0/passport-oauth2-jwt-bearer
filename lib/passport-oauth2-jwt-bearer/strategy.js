  ...
  function doKeyingStep() {
    ...
    try {
      if (self._passReqToCallback) {
        ...
        if (arity == 4) {
          self._keying(req, payload.iss || header.iss, header, keyed);
        } else { // arity == 3
          self._keying(req, payload.iss || header.iss, keyed);
        }
      } else {
        var arity = self._keying.length;
        if (arity == 3) {
          self._keying(payload.iss || header.iss, header, keyed);
        } else { // arity == 2
          self._keying(payload.iss || header.iss, keyed);
        }
      }
    } catch (ex) {
